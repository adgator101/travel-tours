...
const DEMO_BOOKINGS = [
  {
    ...
    packageTitle: "Mountain Adventure Trek",
  ...
  },
  {
    ...
    packageTitle: "European Heritage Tour",
...
function renderBookings() {
  ...
  const slice = filtered.slice(
    (bookingPage - 1) * PER_PAGE,
    bookingPage * PER_PAGE,
  );
  const tbody = document.getElementById("bookingsTableBody");
  ...
  if (!slice.length) {
    ...
    tbody.innerHTML = slice
      .map((b) => {
        const initials = (b.customerName || "U")
          .split(" ")
          .map((w) => w[0])
          .join("")
          .toUpperCase()
...
function showBanner(section, type, msg) {
  const el = document.getElementById(
    (type === "error" ? "errorBanner" : "successBanner") + section,
  );
  if (!el) return;
  el.textContent = msg;
